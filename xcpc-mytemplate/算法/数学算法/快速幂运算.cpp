...
i64 pow_sum(i64 a, int m) // pow_sum(A, m) = sigma pow(A, i)
{						  //                      1<=i<=m
	i64 ans = 0, b = a;
	while (m)
	{
...
{ // 指数在i64范围内
	...
	i64 pow(i64 n)
	{
		return f[0][n & base] * f[1][(n >> 16) & base] % mod * f[2][(n >> 32) & base] % mod * f[3][(n >> 48) & base] % mod;
	}
} power;

int main()
{
	i64 ans = pow_sum(2, 10);
	printf("%lld\n", ans);
	n = 2;
	matrix A;
	A[0][0] = 4;
